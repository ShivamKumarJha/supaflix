...
class XmoviesRepositoryImpl(
    ...
    override fun release(year: String): PagingSource<Int, Contents> {
        return object : PagingSource<Int, Contents>() {
    ...
    override fun country(hash: String, slug: String): PagingSource<Int, Contents> {
        return object : PagingSource<Int, Contents>() {

            override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Contents> {
                return try {
                    val currentPage = params.key ?: 1
                    val response = apiXmovies.country(page = currentPage, hash = hash, slug = slug)
                    Log.d(Constants.TAG, response.body().toString())
                    val contents: ArrayList<Contents> = arrayListOf()
                    val data = response.body()?.contentList ?: emptyList()
                    contents.addAll(data)
    ...
    override fun director(hash: String, slug: String): PagingSource<Int, Contents> {
        return object : PagingSource<Int, Contents>() {

            override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Contents> {
                return try {
                    val currentPage = params.key ?: 1
                    val response = apiXmovies.director(page = currentPage, hash = hash, slug = slug)
                    Log.d(Constants.TAG, response.body().toString())
                    val contents: ArrayList<Contents> = arrayListOf()
                    val data = response.body()?.contentList ?: emptyList()
                    contents.addAll(data)
